...
if (isset($_POST['register'])) {
    ...
    $email_count = mysqli_num_rows($run_check);
    ...
    if ($email_count > 0) {
        ...
        // storing the user profile image path
        $tardir = "img/customer/";
        $fileName = basename($_FILES['pimage']['name']);
        $targetPath = $tardir . $fileName;
        $fileType = pathinfo($targetPath, PATHINFO_EXTENSION);

        // valid extension of image allowed only in jpg, png, jpeg
        ...
        if (in_array($fileType, $allow)) {
            if (move_uploaded_file($_FILES['pimage']['tmp_name'], $targetPath)) {
                ...
                if ($run_insert) {
                    ...
                    // Check if the cart has items for this customer
            ...
            echo "<script>alert('Invalid image file type. Please upload JPG, PNG, or JPEG.')</script>";
        }
    }
}
